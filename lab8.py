...
def dict_fuzzy_search(field: str, searched_value, source_dict):
    estimate = get_estimate_function(field, searched_value)
    index = sorted(source_dict.keys(), key=lambda x: estimate(source_dict[x]))[-1]
    return {**source_dict[index], 'phone': index}

if __name__ == "__main__":
    print('Source:')
    pprint(description_list)
    print('[List] Phone ends with ..3535: ', list_fuzzy_search(PHONE, '3535', description_list))
    
    print('Source:')
    pprint(description_dict)
    print('[Dict] Phone ends with ..3535: ', dict_fuzzy_search('phone', '3535', description_dict))